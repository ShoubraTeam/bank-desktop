...
public class CustomerController {
    ...
    public static void add(ArrayList<String> values) { // ["branch_id", "national_id", "f_name", "l_name", "birthdate", "address", "email", "phone", "dead"]
        String insertSQL = "INSERT INTO customer(branch_id, national_id, f_name, l_name, birthdate, address, email, phone_number, dead) values(?::integer, ?, ?, ?, ?::date, ?, ?, ?, ?::boolean)";
    ...
    public static void QueryOne(int input) {

        int rows;
        int columns = 0;
        String query = "SELECT * FROM customer WHERE joined_at >= CURRENT_DATE - INTERVAL '" + input + " days'";
        try {
    ...
    public static void QueryTwo() {
        int rows;
        int columns = 0;
        String query = "SELECT * FROM customer WHERE national_id NOT IN (SELECT DISTINCT customer_id FROM bank_account)";
        try {
            // Load the MySQL JDBC driver
            Class.forName("org.postgresql.Driver");
