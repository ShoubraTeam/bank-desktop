...
public class DeleteTransactionForm extends Form {
    public DeleteTransactionForm() {
        ...
        for (RowPanelTwoItems row : EntityConstants.TRANSACTIONS_DELETE_ATTRIBUTES) {
            formPanel.add(row);
        }
    ...
    public ActionListener submit() {
        return (e) -> {
            ArrayList<String> values = Helpers.getValuesFromInputs(EntityConstants.TRANSACTIONS_DELETE_ATTRIBUTES);
            TransactionsController.delete(values.get(0));
        };
    }
}
