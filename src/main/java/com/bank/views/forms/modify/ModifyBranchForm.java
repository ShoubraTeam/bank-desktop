...
public class ModifyBranchForm extends Form {
    ...
    public ModifyBranchForm() {
        ...
        for (RowPanelTwoItems row : EntityConstants.BRANCH_MODIFY_ATTRIBUTES) {
            formPanel.add(row);
        }
    ...
    public ActionListener submit() {
        return (e) -> {
            LinkedListPair linkedListPair = Helpers.getValuesFromInputsModify((RowPanelThreeItems<?, ?, ?>[]) EntityConstants.BRANCH_MODIFY_ATTRIBUTES);
            // ADD YOUR CODE HERE
