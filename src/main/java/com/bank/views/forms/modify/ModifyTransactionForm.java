...
public class ModifyTransactionForm extends Form {
    public ModifyTransactionForm() {
        ...
        for (RowPanelTwoItems row : EntityConstants.TRANSACTION_MODIFY_ATTRIBUTES) {
            formPanel.add(row);
        }
    ...
    public ActionListener submit() {
        return (e) -> {
            LinkedHashMap<String,Object> values = Helpers.getValuesFromInputsModify(EntityConstants.TRANSACTION_MODIFY_ATTRIBUTES);
            QueryServices.update(values, TablesName.TRANSACTION);
        };
    }
}
