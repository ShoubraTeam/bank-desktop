...
public class Query10View extends Form {
    ...
    public Query10View() {
        ...
        for (RowPanelTwoItems row : EntityConstants.QUERY_TEN) {
            newRow = row;
    ...
    public ActionListener submit() {
        return (e) -> {
            ArrayList<String> values = Helpers.getValuesFromInputs(EntityConstants.QUERY_TEN);
            ...
            TransactionsController.QueryTen(Integer.parseInt(values.get(0)));

            // إنشاء الجدول بالبيانات
            JTable table = new JTable(new DefaultTableModel(TransactionsController.data,TransactionsController.columns_name));
            JScrollPane scrollPane = new JScrollPane(table); // إضافة التمرير للجدول
            tablePanel.add(scrollPane, BorderLayout.CENTER);
            System.out.println("Data: " + Arrays.toString(TransactionsController.data));
            System.out.println("Columns: " + Arrays.toString(TransactionsController.columns_name));
