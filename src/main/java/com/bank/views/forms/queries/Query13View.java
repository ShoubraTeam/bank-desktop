...
import com.bank.controllers.CreditCardController;
...
public class Query13View extends Form {
    ...
    public Query13View() {
        ...
        for (RowPanelTwoItems row : EntityConstants.QUERY_THIRTEEN) {
            newRow = row;
    ...
    public ActionListener submit() {
        return (e) -> {
            ArrayList<String> values = Helpers.getValuesFromInputs(EntityConstants.QUERY_THIRTEEN);
            ...
            CreditCardController.QueryThirteen(Integer.parseInt(values.get(0)));

            // إنشاء الجدول بالبيانات
            JTable table = new JTable(new DefaultTableModel(CreditCardController.data, CreditCardController.columns_name));
            JScrollPane scrollPane = new JScrollPane(table); // إضافة التمرير للجدول
            tablePanel.add(scrollPane, BorderLayout.CENTER);
            System.out.println("Data: " + Arrays.toString(CreditCardController.data));
            System.out.println("Columns: " + Arrays.toString(CreditCardController.columns_name));
