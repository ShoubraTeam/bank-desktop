...
public class Query14View extends Form {
    ...
    public Query14View() {
        ...
        for (RowPanelTwoItems row : EntityConstants.QUERY_FOURTEEN) {
            newRow = row;
    ...
    public ActionListener submit() {
        return (e) -> {
            ArrayList<String> values = Helpers.getValuesFromInputs(EntityConstants.QUERY_FOURTEEN);
            ...
            ATMController.QueryFourteen(Integer.parseInt(values.get(0)));

            // إنشاء الجدول بالبيانات
            JTable table = new JTable(new DefaultTableModel(ATMController.data, ATMController.columns_name));
            JScrollPane scrollPane = new JScrollPane(table); // إضافة التمرير للجدول
            tablePanel.add(scrollPane, BorderLayout.CENTER);
            System.out.println("Data: " + Arrays.toString(ATMController.data));
            System.out.println("Columns: " + Arrays.toString(ATMController.columns_name));
