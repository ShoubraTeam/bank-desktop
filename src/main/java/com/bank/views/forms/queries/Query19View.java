...
public class Query19View extends Form {
    ...
    public Query19View() {
        ...
        for (RowPanelTwoItems row : EntityConstants.QUERY_NINETEEN) {
            newRow = row;
    ...
    public ActionListener submit() {
        return (e) -> {
            ArrayList<String> values = Helpers.getValuesFromInputs(EntityConstants.QUERY_NINETEEN);
            ...
            BranchController.QueryNineteen(Integer.parseInt(values.get(0)));

            // إنشاء الجدول بالبيانات
            JTable table = new JTable(new DefaultTableModel(BranchController.data, BranchController.columns_name));
            JScrollPane scrollPane = new JScrollPane(table); // إضافة التمرير للجدول
            tablePanel.add(scrollPane, BorderLayout.CENTER);
            System.out.println("Data: " + Arrays.toString(BranchController.data));
            System.out.println("Columns: " + Arrays.toString(BranchController.columns_name));
